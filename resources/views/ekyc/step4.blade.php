<x-app-layout>
    ...
    <script>
        ...
        // Saat provinsi berubah → filter kota
       ...
       //       kotaSelect.innerHTML += `<option value="${item.kota}">${item.kota}</option>`;
        ...
        // Saat provinsi berubah → filter kota unik
document.getElementById('provinsi').addEventListener('change', function () {
    const prov = this.value;
    const kotaSelect = document.getElementById('kota');
    ...
    // Ambil kota berdasarkan provinsi lalu di-unique
    const filteredKota = alamatData
        .filter(item => item.provinsi === prov)
        .map(item => item.kota);

    const uniqueKota = [...new Set(filteredKota)]; // ❗ menghapus duplikat
    ...
    uniqueKota.forEach(kota => {
        ...
        // Saat kota berubah → filter kecamatan
...
// Saat kota berubah → filter kecamatan unik
document.getElementById('kota').addEventListener('change', function () {
    const kota = this.value;
    const kecSelect = document.getElementById('kecamatan');
    ...
    const filteredKec = alamatData
        .filter(item => item.kota === kota)
        .map(item => item.kecamatan);

    const uniqueKec = [...new Set(filteredKec)]; // ❗ menghapus duplikat
    ...
    uniqueKec.forEach(kec => {
        ...
        // Saat kecamatan berubah → tampilkan kode pos
